...
Public Class ctlSprintView
    ...
    Private maryDiaryItems(,) As String ' 1=line, 2=column, 3=date (31 Dec 2017), 4=value, 5=new value
    Public Sub DiaryItemArray(ByVal aryX(,) As String)
        If aryX Is Nothing Then
            ReDim maryDiaryItems(0 To 4, 0 To 1)
        Else
            maryDiaryItems = aryX.Clone
        End If
    End Sub

    Private maryRowHeaders() As String ' 1=value
    Private mlngRows As Integer
    Public Sub RowHeadersArray(ByVal aryX() As String)
        If aryX Is Nothing Then
            ReDim maryRowHeaders(0 To 1)
            mlngRows = 0
        Else
            maryRowHeaders = aryX.Clone
            mlngRows = UBound(aryX)
        End If
    End Sub

    Private Sub DrawSprint(ByVal g As Graphics)
        '
        ...
        ' get the widest row header
        Dim myLongestRowHeader As String
        Dim myLongestRowWidth As Long
        myLongestRowHeader = ""
        myLongestRowWidth = 0
        For lngRow = 0 To UBound(maryRowHeaders)
            myString = maryRowHeaders(lngRow)
            textSize = g.MeasureString(myString, myFont)
            If textSize.Width > myLongestRowWidth Then
                myLongestRowWidth = textSize.Width
                myLongestRowHeader = myString
            End If
        Next

        sngSmallSize = sngFontSize * 0.75
        mySmallFont = New Font(Me.Font.FontFamily, sngSmallSize, FontStyle.Regular)
        textSize = g.MeasureString(myLongestRowHeader, myFont)
        ...
        dblXDivision = (lngScaleWidth - 3) / 14
        dblYDivision = (lngScaleHeight - 2) / UBound(maryDiaryItems, 2)
        ...
        Do While sngFontSize > 4 And (textSize.Width > dblXDivision Or textSize.Height > dblYDivision)
            ...
            dblXDivision = (lngScaleWidth) / 14
            dblYDivision = (lngScaleHeight) / mlngRows
        Loop
